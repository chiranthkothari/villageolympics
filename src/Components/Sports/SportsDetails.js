...
function SportsDetails({ match }) {
  const sportFromUrl = match.params.sport;
  const indexOfSport = sportsData.findIndex((s) => s.title === sportFromUrl);
  const sport = sportsData[indexOfSport];
    ...
    <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
      <div className="flex flex-col items-center justify-between w-full mb-10 lg:flex-row">
          ...
          <div className="max-w-xl mb-6">
            ...
            <h2 className="font-sans text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl sm:leading-none max-w-lg mb-6">
              {sport.title.charAt(0).toUpperCase() +
                sport.title.substr(1).toLowerCase().replaceAll("-", " ")}
            </h2>
        ...
        <div className="flex items-center justify-center lg:w-1/2">
          <div className="w-2/5">
            <img className="object-cover" src={sport.icon} alt="" />
          </div>
        </div>
      </div>
      <a
